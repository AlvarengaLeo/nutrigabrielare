          ...
          <div className="flex flex-wrap items-center justify-center border-b border-primary/10 pb-8 mb-12 gap-8 md:gap-16 tienda-hero-el mt-8">
            ...
            {categories.map((cat) => (
              <button
                ...
                className={`text-lg md:text-xl tracking-tight font-body transition-colors duration-300 ${activeCategory === cat.id ? 'text-primary font-bold' : 'text-primary/40 hover:text-primary/80'}`}
              >
                {cat.title}
              </button>
            ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 border-t border-l border-primary/10">
            {loadingProducts ? (
              ...
              <div className="col-span-full py-24 text-center">
                <p className="font-body text-primary/40 text-lg">No hay productos aún.</p>
              </div>
            )}
          </div>
        </div>
